...
export function useUpdateZonePolygon() {
  ...
  return useMutation({
    ...
    onSuccess: () => {
      qc.invalidateQueries({ queryKey: ['zones'] });
      toast.success('Contour de zone mis à jour');
    },
    onError: (err) => toast.error(errorMessage(err, 'Erreur lors de la mise à jour du contour')),
  });
}
...
export function useSetZoneGateAccess() {
  ...
  return useMutation({
    ...
    onSuccess: () => {
      qc.invalidateQueries({ queryKey: ['zones'] });
      toast.success('Accès portes mis à jour');
    },
    onError: (err) => toast.error(errorMessage(err, 'Erreur lors de la mise à jour des accès')),
  });
}
